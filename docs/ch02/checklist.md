...
- [ ] 配置校验：缺密钥/非法 protocol/文件缺失时给出可读错误并非零退出，无 panic 堆栈（验证：删字段/改 protocol/删文件分别运行）。(AC1/N4)
...
- [ ] 错误反馈：错误 key/不存在模型时，错误在对话区可区分样式显示且不退出（验证：改坏 key 运行后再正常发一条）。(AC11/F11)
- [ ] 退出：`/exit` 与 Ctrl+C 均能安全退出，终端恢复正常（验证：两种方式各试一次，观察无残留/错乱）。(AC10/F10/N7)
- [ ] 界面布局：启动含猫 banner + 名称版本 + cwd + 就绪提示行 + `❯` 占位输入框 + 状态栏（左 name 右 model）（验证：启动截图比对）。(AC7/F7)

## 集成
- [ ] tui 通过统一 `Provider` 接口驱动两种协议，切换协议不改变上层交互（验证：分别用 anthropic / openai 配置跑同一组对话，行为一致）。(AC3/N3)
- [ ] 多轮上下文携带：先告知信息、后追问，模型能正确引用前文；退出再启动后历史为空（验证：两轮对话 + 重启验证）。(AC6/F6)
- [ ] 流式不阻塞：等待/流式期间界面仍响应、不冻结（验证：长回复期间界面持续刷新）。(AC13/N1)
- [ ] scrollback 渲染（Claude Code 风格）：完成的消息（用户输入/助手回复/错误）提交到终端原生滚动历史，可用终端滚轮回看，退出后内容保留；动态区仅含输入框 + 正在流式的回复 + 状态栏（验证：tmux 多轮后回滚查看历史 + 退出后历史仍在）。
...
- [ ] `go build ./...` 无错误。
- [ ] `go vet ./...` 无告警。
- [ ] `go test ./...` 通过（config、conversation 单测）。
