...
- [ ] 在 builtins.go 中临时把某条命令注册两次,`go run ./cmd/mewcode` 立即 panic 退出,panic 文本含具体冲突的命令名(验证:人工临时改动 + 跑 mewcode + 还原)

## 端到端场景(tmux 实跑)

> 验证目标:在真实 tmux 会话内运行 mewcode 二进制,按 plan/spec 描述跑完整流程。
> 准备:`go build -o /tmp/mewcode ./cmd/mewcode`;开一个 `~/.mewcode/memory/MEMORY.md` 保证 /memory 至少能列一条;`SESSIONS_DIR=$PWD/.mewcode/sessions`(按 mewcode 实际默认目录;如未自定义则使用 `${MEWCODE_HOME:-$PWD/.mewcode}/sessions`)。

### 场景 A: 启动与 /help- [ ] **A1** 启动会话:`tmux new-session -d -s mewspec -x 200 -y 50 "/tmp/mewcode 2>/tmp/mewcode.err"`;`sleep 2`;`tmux capture-pane -t mewspec -p | grep -E "(MewCode|Send a message|DEFAULT|PLAN|default|plan)"` 至少命中 2 个关键字
...
### 场景 H: 启动期冲突检测- [ ] **H1** 临时编辑 `internal/command/builtins.go`,把 /help 命令再注册一次同名;`go run ./cmd/mewcode 2>&1 | head -5`;输出含 "panic" 与 "help"(或具体冲突文本);还原 builtins.go
