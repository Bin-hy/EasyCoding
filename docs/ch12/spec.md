# Hook 生命周期挂钩系统 Spec## 背景ch11 把可复用 SOP 搬出源码做成 Skill 包之后,MewCode 在"用户怎么扩展行为"这条路径上还差最后一环:**在 Agent 生命周期的固定时刻自动跑一段用户配置的动作**。当前的扩展点都是显式触发——Skill 要 `/<name>` 唤起、Slash 命令要用户手敲。如果想做这种"触发条件明确、动作固定"的重复事,只能每次手动来:
